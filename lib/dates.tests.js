const { today, tomorrow, yesterday, next } = require('./dates')
const { convert, to, toDays, toLowerCase, toMilliseconds } = require('./dates/convert')
const { addDays, format } = require('./dates/functions')
...
console.log(convert(next(4), format('cccc'), toLowerCase))
console.log(convert(new Date(), toDays, toLowerCase))
console.log(convert(next(3), toMilliseconds))
console.log(convert(next(4), addDays(4), toDays))
